...
const Navbar = () => {
    ...
    return (
        <div className='nav'>
            <div className="navbar bg-green-500 lg:py-4">
                <div className="navbar-start">
                    ...
                    <Link onClick={handleScroll} to='/' className={`${pathName.pathname === '/' || '/loadmore' ? 'text-yellow-200 normal-case text-2xl lg:mx-10' : 'text-white normal-case text-2xl lg:mx-10'}`}>VAPELAND</Link>
                </div>
            ...
            {user ? toggle && <div>
                <h1 className='text-end lg:font-bold pointer bg-green-500 text-white pr-2 lg:pr-4 '><button onClick={logout}>LogOut</button></h1>
            </div> : ''}
        </div>
    );
};
